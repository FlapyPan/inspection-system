  ...
  <div>
    ...
    <el-table :data="tableData" :stripe="true" border style="width: 100%">
      <template v-for="f in fields">
        <template v-if="f.name!=='id'&&f.name!=='roles'&&userStore.hasAnyRoles(f.roles)">
          <template v-if="f.name==='approvalStatus'">
            <el-table-column :label="f.description" :prop="f.name" :width="90" align="center">
              <template #default="scope">
                ...
                <el-tag disable-transitions v-else-if="scope.row['approvalStatus']+'' === '1'" type="success">通过
                </el-tag>
          ...
          <template v-else-if="f.type==='date'">
            <el-table-column :label="f.description" :prop="f.name">
              <template #default="scope">
